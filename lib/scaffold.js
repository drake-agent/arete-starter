...
export async function scaffold({ config, templateDir, dryRun = false }) {
  ...
  const personalKbRoot = join(workspacePath, 'drake-compass');
  const vars = {
    NAME: name,
    USER_NAME: name,
    ...
    PERSONAL_KB_ROOT: personalKbRoot,
  ...
  // Seed PersonalKB pipeline templates into drake-compass/
  const personalKbTemplateDir = join(templateDir, 'personal-kb');
  if (existsSync(personalKbTemplateDir)) {
    const personalKbDirs = [
      personalKbRoot,
      join(personalKbRoot, 'scripts'),
      join(personalKbRoot, 'prompts'),
      join(personalKbRoot, 'wiki'),
      join(personalKbRoot, 'wiki', 'people'),
      join(personalKbRoot, 'wiki', 'companies'),
      join(personalKbRoot, 'wiki', 'projects'),
      join(personalKbRoot, 'wiki', 'meetings'),
      join(personalKbRoot, 'wiki', 'ideas'),
      join(personalKbRoot, 'wiki', 'patterns'),
      join(personalKbRoot, 'wiki', 'insights'),
      join(personalKbRoot, 'wiki', 'decisions'),
      join(personalKbRoot, 'data'),
      join(personalKbRoot, 'data', 'chat-signals'),
      join(personalKbRoot, 'data', 'meeting-signals'),
      join(personalKbRoot, 'data', 'schedule-signals'),
      join(personalKbRoot, 'data', 'normalized'),
      join(personalKbRoot, 'data', 'intel'),
      join(personalKbRoot, 'data', 'pre-brief'),
      join(personalKbRoot, 'data', 'daily-snapshots'),
      join(personalKbRoot, 'data', 'weekly-snapshots'),
      join(personalKbRoot, 'data', 'action-graphs'),
      join(personalKbRoot, 'data', 'merged-signals'),
      join(personalKbRoot, 'data', 'identity-state'),
      join(personalKbRoot, 'data', 'brain'),
      join(personalKbRoot, 'data', 'quality'),
      join(personalKbRoot, 'data', 'drift'),
      join(personalKbRoot, 'data', 'voice-inbox'),
      join(personalKbRoot, 'data', 'saju-filter'),
    ];

    for (const dir of personalKbDirs) {
      if (dryRun) {
        console.log(chalk.yellow(`  [dry-run] Would create dir: ${dir}`));
      } else {
        mkdirSync(dir, { recursive: true });
    ...
    const copyRenderedTemplates = (srcDir, destDir) => {
      if (!existsSync(srcDir)) return;
      for (const entry of readdirSync(srcDir, { withFileTypes: true })) {
        const srcPath = join(srcDir, entry.name);
        if (entry.isDirectory()) {
          copyRenderedTemplates(srcPath, join(destDir, entry.name));
          continue;
        }
        ...
        if (dryRun) {
          ...
          writeFileSync(outputPath, rendered, 'utf-8');
          console.log(chalk.dim(`  Created: ${outputPath.replace(`${workspacePath}/`, '')}`));
        }
      }
    };

    copyRenderedTemplates(personalKbTemplateDir, personalKbRoot);
  }
...
`;
  ...
  const gsEnvPath = join(gatsaengDir, '.env');
  writeFileSync(gsEnvPath, envContent, { encoding: 'utf-8', mode: 0o600 });
  console.log(chalk.dim('  Created gatsaeng-os/.env (mode 0600)'));
