...
export async function runWizard({ dryRun = false } = {}) {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      filter: (v) => v.startsWith('~') ? resolve(homedir(), v.slice(1)) : v,
    },
    {
      type: 'input',
      name: 'timezone',
  ...
  if (answers.sajuCoaching) {
    const sajuAnswers = await inquirer.prompt([
      {
        ...
        validate: (v) => {
          ...
          return !isNaN(n) && n >= 1900 && n <= 2026 ? true : 'Enter a valid year (1900-2026)';
        },
      ...
      },
      {
        ...
        validate: (v) => {
          const n = parseInt(v, 10);
          return !isNaN(n) && n >= 1 && n <= 12 ? true : 'Enter 1-12';
        },
        filter: (v) => String(parseInt(v, 10)),
      },
      {
        ...
        validate: (v) => {
          const n = parseInt(v, 10);
          return !isNaN(n) && n >= 1 && n <= 31 ? true : 'Enter 1-31';
        },
        filter: (v) => String(parseInt(v, 10)),
      },
      {
    ...
    const parsedHour = parseInt(sajuAnswers.birthHour, 10);
    sajuProfile = {
      year: parseInt(sajuAnswers.birthYear, 10),
      month: parseInt(sajuAnswers.birthMonth, 10),
      day: parseInt(sajuAnswers.birthDay, 10),
      hour: sajuAnswers.birthHour === 'unknown' || isNaN(parsedHour) ? null : parsedHour,
